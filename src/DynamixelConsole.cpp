...
DynamixelConsole::DynamixelConsole(DynamixelInterface &aInterface, Stream &aConsole) : mInterface(aInterface), mConsole(aConsole)
{
...
void DynamixelConsole::loop()
{
    ...
    {
        if (c >= 32 && c <= 126 && size_t(mLinePtr - &(mLineBuffer[0])) < sLineBufferSize)
        {
            mConsole.write(c);
            *mLinePtr = c;
            ++mLinePtr;
...
{
    if (aStatus == DynStatus::INTERNAL_ERROR)
    {
        mConsole.print("Invalid command parameters\n\r");
        return;
    }
    mConsole.print("Status : ");
    if (aStatus == DynStatus::OK)
    {
        mConsole.print("ok");
    }
    else if (aStatus & DynStatus::COM_ERROR)
    {
        mConsole.print("communication error");
        if (aStatus & DynStatus::TIMEOUT)
        {
            mConsole.print(", timeout");
        }
        else if (aStatus & DynStatus::CHECKSUM_ERROR)
        {
    ...
    else
    {
        ...
        if (aStatus & DynStatus::INPUT_VOLTAGE_ERROR)
        {
            mConsole.print(", invalid input voltage");
        }
        if (aStatus & DynStatus::ANGLE_LIMIT_ERROR)
        {
            mConsole.print(", angle limit error");
        }
        if (aStatus & DynStatus::OVERHEATING_ERROR)
        {
            mConsole.print(", overheating");
        }
        if (aStatus & DynStatus::RANGE_ERROR)
        {
            mConsole.print(", out of range value");
        }
        if (aStatus & DynStatus::CHECKSUM_ERROR)
        {
            mConsole.print(", invalid command checksum");
        }
        if (aStatus & DynStatus::OVERLOAD_ERROR)
        {
            mConsole.print(", overload");
        }
        if (aStatus & DynStatus::INSTRUCTION_ERROR)
        {
...
DynamixelStatus DynamixelConsole::ping(int argc, char **argv)
{
    ...
    if (argc < 2)
    {
        ...
        return DynStatus::INTERNAL_ERROR;
    }
    id = atoi(argv[1]);
    if (id > 254)
    {
