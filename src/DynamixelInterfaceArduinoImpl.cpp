...
template <class T>
DynamixelInterfaceImpl<T>::DynamixelInterfaceImpl(T &aStream, uint8_t aTxPin, uint8_t aDirectionPin) : mStream(aStream), mDirectionPin(aDirectionPin), mTxPin(aTxPin)
{
    if (mDirectionPin != NO_DIR_PORT)
    {
        digitalWrite(mDirectionPin, LOW);
...
HardwareDynamixelInterface::HardwareDynamixelInterface(HardwareSerial &aSerial, uint8_t aDirectionPin) : DynamixelInterfaceImpl(aSerial, TxPinFromHardwareSerial(aSerial), aDirectionPin)
{
}
...
SoftwareDynamixelInterface::SoftwareDynamixelInterface(uint8_t aRxPin, uint8_t aTxPin, uint8_t aDirectionPin) : DynamixelInterfaceImpl(mSoftSerial, aTxPin, aDirectionPin),
